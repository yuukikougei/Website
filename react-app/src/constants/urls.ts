/**
 * 外部URLの定数定義
 * @description このファイルは外部リンクのURLを一元管理します
 */

/**
 * ソーシャルメディア関連のURL
 */
export const SOCIAL_MEDIA_URLS = {
  INSTAGRAM: "https://www.instagram.com/yuukikougei_?igsh=c2xmNjJrajN2ZnA1",
} as const;

/**
 * 地図・位置情報関連のURL
 */
export const MAP_URLS = {
  GOOGLE_MAPS_EMBED: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3358.241086290764!2d129.81670497570863!3d32.67963407370458!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x351551d82b4f3603%3A0x5b3ccba1a32ec8d3!2z44CSODUxLTAzMDEg6ZW35bSO55yM6ZW35bSO5biC5rex5aCA55S677yW5LiB55uu77yU77yT77yS!5e0!3m2!1sja!2sjp!4v1757145782585!5m2!1sja!2sjp",
} as const;

/**
 * 求人・採用関連のURL
 * @note これらのURLは定期的に更新が必要です
 */
export const JOB_RECRUITMENT_URLS = {
  SHIP_CARPENTER: "https://www.hellowork.careers/%E3%82%86%E3%81%83%E3%81%8D%E5%B7%A5%E8%8A%B8%E3%81%A7%E3%81%AE%E8%88%B9%E8%88%B6%E7%8F%BE%E5%A0%B4%E5%A4%A7%E5%B7%A5%EF%BC%88%E5%86%85%E8%A3%85%E5%8F%8A%E3%81%B3%E6%8D%AE%E4%BB%98%EF%BC%89%E3%81%AE%E3%83%8F%E3%83%AD%E3%83%BC%E3%83%AF%E3%83%BC%E3%82%AF%E6%B1%82%E4%BA%BA%E6%83%85%E5%A0%B1%EF%BC%88%E9%95%B7%E5%B4%8E%E7%9C%8C%E9%95%B7%E5%B4%8E%E5%B8%82%EF%BC%89-%2042010-13269151/",
  FURNITURE_CRAFTSMAN: "https://www.hellowork.careers/%E3%82%86%E3%81%83%E3%81%8D%E5%B7%A5%E8%8A%B8%E3%81%A7%E3%81%AE%E5%AE%B6%E5%85%B7%E8%81%B7%E4%BA%BA%EF%BC%88%E8%88%B6%E8%88%B6%E5%AE%B6%E5%85%B7%EF%BC%89%E3%81%AE%E3%83%8F%E3%83%AD%E3%83%BC%E3%83%AF%E3%83%BC%E3%82%AF%E6%B1%82%E4%BA%BA%E6%83%85%E5%A0%B1%EF%BC%88%E9%95%B7%E5%B4%8E%E7%9C%8C%E9%95%B7%E5%B4%8E%E5%B8%82%EF%BC%89-%2042010-13270551/",
} as const;
